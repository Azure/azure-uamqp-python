...
- Fixed bug in dropping recevied messages at the moment when the connection just started working.
- Fixed bug where underlying io type wasn't set to WebSocket when http_proxy was appied (PR#92, Thanks to skoop22).
...
- Added method `work` to `MessageReceiver` and `work_async` to `MessageReceiverAsync` responsbile for updating link status.
