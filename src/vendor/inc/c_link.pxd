...
cdef extern from "azure_uamqp_c/link.h":
    ...
    cdef enum LINK_STATE_TAG:
        LINK_STATE_DETACHED = 1,    # because C code uses `MU_DEFINE_ENUM`
    ...
    cdef enum LINK_TRANSFER_RESULT_TAG:
        LINK_TRANSFER_ERROR = 1,    # because C code uses `MU_DEFINE_ENUM`
        LINK_TRANSFER_BUSY

    cdef enum LINK_DELIVERY_SETTLE_REASON_TAG:
        LINK_DELIVERY_SETTLE_REASON_DISPOSITION_RECEIVED = 1, # because C code uses `MU_DEFINE_ENUM`
