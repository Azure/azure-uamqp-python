...
def main():
    ...
    with Connection(
        ...
        with c.create_session(
            ...
            # mgmt_link = session.create_request_response_link_pair("$management")
            # mgmt_link.open()
            #
            # while mgmt_link.state.value != 3:
            #     c.listen()
            #
            # mgmt_link.close()

            mgmt_msg = Message(
                properties=Properties(message_id=0),
                application_properties={
                    "name": config['event_hub'],
                    "partition": "0",
                    "operation": "READ",
